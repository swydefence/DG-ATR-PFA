...
import train_utils
import mixstyle.resnet as models
...
def main(args: argparse.Namespace):
    ...
    # Data loading code
    train_transform = train_utils.get_S2M_train_transform(args.train_resizing, random_horizontal_flip=True,
                                                random_color_jitter=True, random_gray_scale=True)
    val_transform = train_utils.get_S2M_val_transform(args.val_resizing)
    print("train_transform: ", train_transform)
    print("val_transform: ", val_transform)

    train_dataset, num_classes,args.class_names = train_utils.get_dataset(dataset_name=args.data, root=args.root, task_list=args.sources,
                                                   split='train', download=True, transform=train_transform,
                                                   seed=args.seed)
    sampler = train_utils.RandomDomainSampler(train_dataset, args.batch_size, n_domains_per_batch=3)
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, num_workers=args.workers,
                              sampler=sampler, drop_last=True)
    # val_dataset, _,args.class_names = utils.get_dataset(dataset_name=args.data, root=args.root, task_list=['S'], split='train',
    #                                    download=True, transform=val_transform, seed=args.seed)
    val_dataset, _,args.class_names = train_utils.get_dataset(dataset_name=args.data, root=args.root, task_list=args.sources, split='val',
                                       download=True, transform=val_transform, seed=args.seed)

    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers)
    test_dataset, _ ,args.class_names = train_utils.get_dataset(dataset_name=args.data, root=args.root, task_list=args.targets, split='test',
                                        download=True, transform=val_transform, seed=args.seed)
    

    # noise_dataset,_,_= utils.get_dataset(dataset_name=args.data, root=args.root, task_list='S', split='val',
    #                                    download=True, transform=val_transform, seed=args.seed)
    ...
    classifier = train_utils.ImageClassifier(backbone, num_classes, freeze_bn=args.freeze_bn, dropout_p=args.dropout_p,
    ...
    if args.phase == 'analysis':
        ...
        source_feature = train_utils.collect_feature(val_loader, feature_extractor, device, max_num_features=800)
        target_feature = train_utils.collect_feature(test_loader, feature_extractor, device, max_num_features=800)
    ...
    if args.phase == 'test':
        acc1 = train_utils.validate(test_loader, classifier, args, device)
        print(acc1)
        return
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data', metavar='DATA', default='PACS',
                        help='dataset: ' + ' | '.join(train_utils.get_dataset_names()) +
                             ' (default: PACS)')
