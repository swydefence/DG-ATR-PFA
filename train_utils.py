...
sys.path.append('../../..')
from modules import Classifier as ClassifierBase
...
def get_dataset(dataset_name, root, task_list, split='train', download=True, transform=None, seed=0):
    ...
    supported_dataset = ['PACS', 'OfficeHome', 'DomainNet','Sample','S2M','mstareoc']
    ...
    for task in task_list:
        ...
        train_split_list.append(all_split)
        val_split_list.append(all_split)
        test_split_list.append(all_split)
    ...
    return dataset_dict[split], num_classes, class_names
...
def get_S2M_train_transform(resizing='default', random_horizontal_flip=True, random_color_jitter=True,
                        random_gray_scale=True):
...
def get_S2M_val_transform(resizing='default'):
